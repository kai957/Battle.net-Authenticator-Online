...
//include_once '../classes/Proxy.class.php';
...
function queryArray($sql) {
    ...
    for ($i = 0, $row = mysqli_fetch_array($res); $row != false; $i++, $row = mysqli_fetch_array($res)) {
        $t[$i] = $row;
    }
    if (count($t) < 1) {
        return false;
...
function _cget($url = '', $proxy = '', $useragent = '', $username = '', $password = '') {
    if (!$url) {
        return "";
    }
    $ch = curl_init();
...
function checkOneKeyCommitJsonArrayVaild($jsonArray) {
    global $user;
    if ($jsonArray == null) {
        return FALSE;
    }
    ...
    if (ctype_digit($auth_id)) {//不是整数不通过，防止SQL注入
...
function registerOnKeyLogin($authenticator) {
