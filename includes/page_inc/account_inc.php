...
function allatthecho($login, $userid, $userright) {
    ...
    if ($login == 0) {
        ...
        foreach ($results as $rowarr) {
            if ($rowarr['auth_img'] < 1 || $rowarr['auth_img'] > 6 || $rowarr['auth_img'] != intval($rowarr['auth_img'])) {
                $rowarr['auth_img'] = 0;
            }
            $returntxt = $returntxt . '
            <li class="border-4">
            ...
            <div id="lobby"><div id="page-content" class="page-content">
                    <div id="lobby-account">
                        ...
                        <div class="lobby-box security-box">
                            ...
                            <?php
                            if (USER_DONATED) {
                                if ($auth_total_all >= 0 && $auth_total_all <= (int) (AUTH_HINT_LOGO_XISHU_DONATED * MOST_AUTH_DONATED / 3)) {
                                    echo "<p class='has-authenticator-has-active'>$auth_total_all/" . MOST_AUTH_DONATED . "</p>";
                                } elseif ($auth_total_all >= (int) (AUTH_HINT_LOGO_XISHU_DONATED * MOST_AUTH_DONATED / 3) + 1 && $auth_total_all <= (MOST_AUTH_DONATED - 1)) {
                                    echo "<p class='has-authenticator-none-active'>$auth_total_all/" . MOST_AUTH_DONATED . "</p>";
                                } else {
                                    echo "<p class='none-authenticator'>$auth_total_all/" . MOST_AUTH_DONATED . "</p>";
                                }
                            } else {
                                if (MOST_AUTH > 1) {
                                    if ($auth_total_all >= 0 && $auth_total_all <= (int) (AUTH_HINT_LOGO_XISHU * MOST_AUTH / 3)) {
                                        echo "<p class='has-authenticator-has-active'>$auth_total_all/" . MOST_AUTH . "</p>";
                                    } elseif ($auth_total_all >= (int) (AUTH_HINT_LOGO_XISHU * MOST_AUTH / 3) + 1 && $auth_total_all <= (MOST_AUTH - 1)) {
                    ...
                    <div id="lobby-games">
                        ...
                        <div id="games-list"> 
                            ...
                            <ul id="game-list-wow" style="display: none;">
                                ...
                                if ((USER_DONATED && $resultnumber < MOST_AUTH_DONATED) || (!USER_DONATED && $resultnumber < MOST_AUTH)) {
                                    ?>
                                    <!--添加账号-->
                        ...
                        <div id="games-tools">
                            <?php if ((USER_DONATED && $resultnumber < MOST_AUTH_DONATED) || (!USER_DONATED && $resultnumber < MOST_AUTH)) { ?>
